...
if [ -n "${INITRD}" ]; then
	INITRD_NODE="
		initrd${REFERENCE_CHAR}$INITRDNUM {
			description = \"${ARCH_UPPER} libreCMC ${DEVICE} initrd\";
			${COMPATIBLE_PROP}
			data = /incbin/(\"${INITRD}\");
			type = \"ramdisk\";
			arch = \"${ARCH}\";
...
if [ -n "${ROOTFS}" ]; then
	...
	ROOTFS_NODE="
		rootfs${REFERENCE_CHAR}$ROOTFSNUM {
			description = \"${ARCH_UPPER} libreCMC ${DEVICE} rootfs\";
			${COMPATIBLE_PROP}
...
[ "$DTOVERLAY" ] && for overlay in $DTOVERLAY ; do
	...
	FDTOVERLAY_NODE="$FDTOVERLAY_NODE
		...
		$ovnode {
			description = \"${ARCH_UPPER} libreCMC ${DEVICE} device tree overlay $ovname\";
			${COMPATIBLE_PROP}
...
"
	OVCONFIGS="$OVCONFIGS
		...
		$ovname {
			description = \"libreCMC ${DEVICE} overlay $ovname\";
			fdt = \"$ovnode\";
			${COMPATIBLE_PROP}
		};
	"
