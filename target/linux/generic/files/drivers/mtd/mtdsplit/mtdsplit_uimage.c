			    ...
			    u32 *header_offset, u32 *part_magic)
{
	...
	if (!np || !of_device_is_compatible(np, "librecmc,uimage"))
		return;

	if (!of_property_read_u32(np, "librecmc,padding", extralen))
		pr_debug("got librecmc,padding=%d from device-tree\n", *extralen);
	if (!of_property_read_u32(np, "librecmc,ih-magic", ih_magic))
		pr_debug("got librecmc,ih-magic=%08x from device-tree\n", *ih_magic);
	if (!of_property_read_u32(np, "librecmc,ih-type", ih_type))
		pr_debug("got librecmc,ih-type=%08x from device-tree\n", *ih_type);
	if (!of_property_read_u32(np, "librecmc,offset", header_offset))
		pr_debug("got ih-start=%u from device-tree\n", *header_offset);
	if (!of_property_read_u32(np, "librecmc,partition-magic", part_magic))
		pr_debug("got librecmc,partition-magic=%08x from device-tree\n", *part_magic);
}
