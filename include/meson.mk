...
MESON_HOST_BUILD_DIR:=$(HOST_BUILD_DIR)/librecmc-build
MESON_HOST_VARS:=
MESON_HOST_ARGS:=

MESON_BUILD_DIR:=$(PKG_BUILD_DIR)/librecmc-build
MESON_VARS:=
MESON_ARGS:=
...
define Meson/CreateNativeFile
	$(STAGING_DIR_HOST)/bin/sed \
		...
		< $(MESON_DIR)/librecmc-native.txt.in \
		> $(1)
endef
...
define Meson/CreateCrossFile
	$(STAGING_DIR_HOST)/bin/sed \
		...
		< $(MESON_DIR)/librecmc-cross.txt.in \
		> $(1)
endef

define Host/Configure/Meson
	$(call Meson/CreateNativeFile,$(HOST_BUILD_DIR)/librecmc-native.txt)
	$(call Meson, \
		--native-file $(HOST_BUILD_DIR)/librecmc-native.txt \
		$(MESON_HOST_ARGS) \
...
define Build/Configure/Meson
	$(call Meson/CreateNativeFile,$(PKG_BUILD_DIR)/librecmc-native.txt)
	$(call Meson/CreateCrossFile,$(PKG_BUILD_DIR)/librecmc-cross.txt)
	$(call Meson, \
		--buildtype plain \
		--native-file $(PKG_BUILD_DIR)/librecmc-native.txt \
		--cross-file $(PKG_BUILD_DIR)/librecmc-cross.txt \
		$(MESON_ARGS) \
		$(MESON_BUILD_DIR) \
		$(MESON_BUILD_DIR)/.., \
		$(MESON_VARS))
